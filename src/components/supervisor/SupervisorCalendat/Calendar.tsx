...
export const Calendar = ({ ptos }: Props) => {
  ...
  const handleScroll: React.UIEventHandler<HTMLDivElement> = event => {
    ...
    if (scrollPercent < 20) {
      ...
      container.scrollTop = container.scrollTop + 512;
    }
  };
            ...
            return (
              ...
              >
                {Array.from({ length: 7 }).map((_, indexNested) => {
                  ...
                  return (
                    <GridItem key={indexNested} colStart={indexNested + 1} px={2}>
                      <HStack w={'100%'}>
                        {day.getDate() != 1 && <Text color={isSunday || isHoliday ? 'red' : ''}>{day.getDate()}</Text>}
                        {day.getDate() === 1 && (
                ...
                {ptosToRenderThisWeek.map(p => {
                  ...
                  if (p.id === 2) {
                    console.log(endingThisWeek);
                    console.log(end);
                    console.log(p.ptoEnd.getDate());
                    console.log(p.ptoEnd.getDay());
                  }
                  return (
